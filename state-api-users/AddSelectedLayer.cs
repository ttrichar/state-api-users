...
namespace AmblOn.State.API.Users
{
    ...
    public class AddSelectedLayer
    {
        ...
        #region Constructors
        public AddSelectedLayer(AmblOnGraph amblGraph)
        {
        ...
        {
            return await stateBlob.WithStateHarness<UsersState, AddSelectedLayerRequest, UsersStateHarness>(req, signalRMessages, log,
                async (harness, reqData, actReq) =>
            {
                log.LogInformation($"AddSelectedLayer");

                var stateDetails = StateUtils.LoadStateDetails(req);

                //await harness.AddSelectedLayer(amblGraph, stateDetails.Username, stateDetails.EnterpriseLookup, reqData.LayerID);
