...
namespace AmblOn.State.API.Users
{
    ...
    public class EditPhoto
    {
        ...
        #region Constructors
        public EditPhoto(AmblOnGraph amblGraph)
        {
        ...
        {
            return await stateBlob.WithStateHarness<AmblOnState, EditPhotoRequest, AmblOnStateHarness>(req, signalRMessages, log,
                async (harness, reqData, actReq) =>
            {
                log.LogInformation($"EditPhoto");

                var stateDetails = StateUtils.LoadStateDetails(req);

                await harness.EditPhoto(amblGraph, stateDetails.Username, stateDetails.EnterpriseAPIKey, reqData.Photo, reqData.AlbumID);
